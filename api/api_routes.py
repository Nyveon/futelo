...
import api.crud as crud  # Import CRUD functions
...
def process_message(
    request: MessageProcessRequest, session: Session = Depends(get_session)
):
    """
    ...
    last_user_message = crud.get_or_create_last_user_message(
        session, request.telegram_group_id
    )

    is_valid_consecutive, reason, needed_currency = check_user_concurrent_message_count(
    ...
    # 2. Update user stats
    user = crud.update_user_after_message(
        session, user, last_user_message, needed_currency
    )

    return ProcessResponse(
        success=True, message=None, lost_currency=(needed_currency > 0)
    )


@router.post("/lootbox/buy", response_model=LootboxBuyResponse)
def process_buy_lootbox(
    request: LootboxBuyRequest, session: Session = Depends(get_session)
):
    ...
    with session.begin():
        ...
        rarity = str(
            random.choice(
                [lootbox["rarity"] for lootbox in config.lootboxes],
                p=[lootbox["probability"] for lootbox in config.lootboxes],
            )
        )

        lootbox = crud.create_lootbox(
            session, request.telegram_user_id, request.telegram_group_id, rarity
        )

        return LootboxBuyResponse(
            success=True, message=None, lootbox_id=lootbox.id, rarity=rarity
        )


@router.post("/lootbox/open", response_model=LootboxOpenResponse)
def process_open_lootbox(
    request: LootboxOpenRequest, session: Session = Depends(get_session)
):
    ...
    with session.begin():
        ...
        if (
            lootbox.telegram_user_id != request.telegram_user_id
            or lootbox.telegram_group_id != request.telegram_group_id
        ):
            return LootboxOpenResponse(
                success=False,
        ...
        user = crud.get_or_create_user(
            session, lootbox.telegram_user_id, lootbox.telegram_group_id
        )
...
def get_user(
    telegram_user_id: int, telegram_group_id, session: Session = Depends(get_session)
):
